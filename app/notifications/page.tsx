...
type Notification = { id: number; type: "message" | "viewing" | "application" | "verification"; title: string; body: string; href: string; readAt: string | null; createdAt: string };
const symbols = { message: "M", viewing: "V", application: "A", verification: "✓" };
...
export default function NotificationsPage() {
  ...
  return <main className="notifications-page"><header className="site-header shell market-header"><Link className="brand" href="/"><span className="brand-logo" /></Link><nav><Link href="/listings">Browse homes</Link><Link href="/applications">Applications</Link><Link href="/viewings">Viewings</Link><Link href="/messages">Messages</Link></nav><Link className="button button-dark" href="/dashboard">Dashboard</Link></header><section className="notifications-shell shell"><div className="notifications-heading"><div><span className="eyebrow">YOUR ACTIVITY</span><h1>Notifications</h1><p>Messages and important updates about applications, viewings, and verification appear here.</p></div>{unreadCount > 0 && <button onClick={() => void markAll()}>Mark all as read</button>}</div>{error && <p className="form-error dashboard-error">{error}</p>}{loading ? <p>Loading notifications…</p> : items.length === 0 ? <div className="empty-state"><strong>You are all caught up.</strong><p>New activity will appear here.</p><Link className="button button-dark" href="/listings">Browse homes</Link></div> : <div className="notification-list">{items.map((item) => <button className={`notification-row ${item.readAt ? "read" : "unread"}`} key={item.id} onClick={() => void open(item)}><span className={`notification-symbol ${item.type}`}>{symbols[item.type]}</span><div><strong>{item.title}</strong><p>{item.body}</p><small>{new Date(item.createdAt).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</small></div>{!item.readAt && <i aria-label="Unread" />}</button>)}</div>}</section></main>;
}
