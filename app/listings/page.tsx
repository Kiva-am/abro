...
export default function ListingsPage() {
  ...
  async function load(values = filters, options: { updateUrl?: boolean; page?: number; append?: boolean } = {}) {
    const { updateUrl = true, page: requestedPage = 1, append = false } = options;
    const requestId = ++loadRequest.current;
    if (append) setLoadingMore(true); else { setLoading(true); setLoadingMore(false); appliedFilters.current = values; }
    setError("");
    const browserQuery = toQuery(values);
    if (updateUrl) window.history.replaceState(null, "", `${window.location.pathname}${browserQuery.size ? `?${browserQuery}` : ""}`);
    const apiQuery = new URLSearchParams(browserQuery);
    if (requestedPage > 1) apiQuery.set("page", String(requestedPage));
    try {
      const response = await fetch(`/api/listings?${apiQuery}`); const text = await response.text();
      const data = text ? JSON.parse(text) as { listings?: Listing[]; error?: string; hasMore?: boolean } : {};
      if (!response.ok) throw new Error(data.error || "Listings could not be loaded.");
      if (requestId !== loadRequest.current) return;
      const nextItems = data.listings ?? [];
      setItems((current) => append ? [...current, ...nextItems.filter((item) => !current.some((existing) => existing.id === item.id))] : nextItems);
      setPage(requestedPage);
      setHasMore(Boolean(data.hasMore));
    } catch {
      if (requestId === loadRequest.current) setError(append ? "More listings could not be loaded right now." : "Listings could not be loaded right now.");
    } finally {
      if (requestId === loadRequest.current) { setLoading(false); setLoadingMore(false); }
    }
  }

  useEffect(() => {
    void load(filters, { updateUrl: false });
  ...
  return <main className="market-page">
    <header className="site-header shell market-header"><Link className="brand" href="/" aria-label="Debal home"><span className="brand-logo" aria-hidden="true" /></Link><nav><Link href="/listings">Browse homes</Link><Link href="/favorites">Saved</Link><Link href="/viewings">Viewings</Link><Link href="/applications">Applications</Link><Link href="/dashboard">My properties</Link><Link href="/onboarding">My profile</Link></nav><div className="header-actions"><NotificationLink /><Link className="button button-dark" href="/listings/new">List a place</Link></div></header>
    ...
    <section className="market-results shell"><div className="results-heading"><h2>Available places</h2><span>{loading ? "Searching…" : `${items.length} listing${items.length === 1 ? "" : "s"}${hasMore ? " loaded" : ""}`}</span></div>{error && <p className="form-error">{error}</p>}{!loading && !error && items.length === 0 && <div className="empty-state"><strong>No listings match these filters.</strong><p>Clear one or two choices to see more homes.</p><button className="button button-dark" onClick={clear}>Clear all filters</button></div>}<div className="market-grid">{items.map((item, index) => <article className="market-card" key={item.id}><div className={`market-photo listing-photo-${index % 3 === 0 ? "one" : index % 3 === 1 ? "two" : "three"}`}>{item.photoKey && <img className="listing-image" src={`/api/media/${item.photoKey}`} alt={item.title} />}<span>{item.verificationStatus === "verified" ? "✓ Verified property" : "New listing"}</span><FavoriteButton listingId={item.id} initialSaved={savedIds.includes(item.id)} compact /><b>{String(index + 1).padStart(2, "0")}</b></div><Link href={`/listings/${item.id}`} className="market-card-body"><span className="area">{item.neighborhood ? `${item.neighborhood}, ` : ""}{item.city}</span><h3>{item.title}</h3><p>{item.description}</p><div className="property-tags"><span>{roomLabels[item.roomType]}</span><span>{item.bedrooms} bed</span><span>{item.furnished ? "Furnished" : "Unfurnished"}</span><span>From {item.availableFrom}</span></div><div className="market-price"><strong>{Number(item.monthlyRent).toLocaleString()} ETB<small>/month</small></strong><span>View details →</span></div></Link></article>)}</div>{!loading && items.length > 0 && hasMore && <div className="load-more-row"><button className="button button-dark" type="button" disabled={loadingMore} onClick={() => void load(appliedFilters.current, { updateUrl: false, page: page + 1, append: true })}>{loadingMore ? "Loading more…" : "Load more homes"}</button></div>}</section>
  </main>;
}
