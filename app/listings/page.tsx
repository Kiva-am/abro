...
type Listing = { id: number; title: string; description: string; monthlyRent: number; deposit: number; roomType: string; bedrooms: number; bathrooms: number; furnished: number; utilitiesIncluded: number; availableFrom: string; verificationStatus: string; city: string; neighborhood: string | null };
...
export default function ListingsPage() {
  ...
  const [items, setItems] = useState<Listing[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  async function load() {
  ...
  return <main className="market-page">
    <header className="site-header shell market-header"><Link className="brand" href="/" aria-label="Debal home"><span className="brand-logo" aria-hidden="true" /></Link><nav><Link href="/listings">Browse homes</Link><Link href="/onboarding">My profile</Link></nav><Link className="button button-dark" href="/listings/new">List a place</Link></header>
    ...
    <section className="market-results shell"><div className="results-heading"><h2>Available places</h2><span>{loading ? "Loading…" : `${items.length} listing${items.length === 1 ? "" : "s"}`}</span></div>{error && <p className="form-error">{error}</p>}{!loading && !error && items.length === 0 && <div className="empty-state"><strong>No listings match yet.</strong><p>Try a wider budget or publish the first place in this area.</p><Link className="button button-dark" href="/listings/new">Create listing</Link></div>}<div className="market-grid">{items.map((item, index) => <article className="market-card" key={item.id}><div className={`market-photo listing-photo-${index % 3 === 0 ? "one" : index % 3 === 1 ? "two" : "three"}`}><span>{item.verificationStatus === "verified" ? "✓ Verified property" : "New listing"}</span><b>0{index + 1}</b></div><div className="market-card-body"><span className="area">{item.neighborhood ? `${item.neighborhood}, ` : ""}{item.city}</span><h3>{item.title}</h3><p>{item.description}</p><div className="property-tags"><span>{roomLabels[item.roomType]}</span><span>{item.bedrooms} bed</span><span>{item.furnished ? "Furnished" : "Unfurnished"}</span></div><div className="market-price"><strong>{Number(item.monthlyRent).toLocaleString()} ETB<small>/month</small></strong><span>Available {item.availableFrom}</span></div></div></article>)}</div></section>
  </main>;
}
