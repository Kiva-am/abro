...
export async function POST(request: Request) {
  ...
  try {
    ...
    const listing = await env.DB.prepare("SELECT id,owner_id AS ownerId FROM listings WHERE id=? AND status='active'")
      .bind(listingId).first<{ id: number; ownerId: string }>();
    ...
    await env.DB.prepare(`INSERT INTO rental_applications
      (listing_id,renter_id,owner_id,message,move_in_date,occupants) VALUES (?,?,?,?,?,?)`)
      .bind(listingId, identity.userId, listing.ownerId, message, moveInDate, occupants).run();
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const item = await env.DB.prepare("SELECT id,listing_id AS listingId,renter_id AS renterId,owner_id AS ownerId,status FROM rental_applications WHERE id=?")
      .bind(id).first<{ id: number; listingId: number; renterId: string; ownerId: string; status: string }>();
    ...
    if (item.ownerId === identity.userId) {
      ...
      if (status === "accepted") {
        const results = await env.DB.batch([update, env.DB.prepare(`UPDATE rental_applications SET status='declined',updated_at=CURRENT_TIMESTAMP
        ...
        if (!result.meta.changes) return Response.json({ error: "This application changed before your decision. Refresh and try again." }, { status: 409 });
      }
    ...
    if (item.renterId === identity.userId) {
      ...
      const result = await env.DB.prepare(`UPDATE rental_applications SET status='withdrawn',updated_at=CURRENT_TIMESTAMP
        WHERE id=? AND renter_id=? AND status IN ('pending','shortlisted')`).bind(id, identity.userId).run();
      if (!result.meta.changes) return Response.json({ error: "This application changed before it could be withdrawn. Refresh and try again." }, { status: 409 });
      return Response.json({ updated: true, status });
    }
